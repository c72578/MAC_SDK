...
extern "C"
{
    ...
    __declspec( dllexport ) int __stdcall ShowFileInfoDialog(const APE::str_ansi * pFilename, HWND hwndWindow);
    __declspec( dllexport ) int __stdcall TagFileSimple(const APE::str_ansi * pFilename, const char * pArtist, const char * pAlbum, const char * pTitle, const char * pComment, const char * pGenre, const char * pYear, const char * pTrack, BOOL bClearFirst, BOOL bUseOldID3);
	__declspec( dllexport ) int __stdcall GetID3Tag(const APE::str_ansi * pFilename, APE::ID3_TAG * pID3Tag);
    __declspec( dllexport ) int __stdcall RemoveTag(const APE::str_ansi * pFilename);
}
